...
describe('calculatorReducer', () => {
  it('returns initial state unchanged for unknown action', () => {
    ...
    expect(calculatorReducer(INITIAL_STATE, { type: 'UNKNOWN' })).toBe(
      INITIAL_STATE,
    );
  });
  ...
  it('SET_VEHICLE_VARIANT clears with null', () => {
    const withVehicle: CalculatorState = {
      ...INITIAL_STATE,
      vehicleVariantId: 'vv-1',
    };
    const next = calculatorReducer(withVehicle, {
      type: 'SET_VEHICLE_VARIANT',
      id: null,
    });
  ...
  it('ADD_ACCESSORY is idempotent for same accessoryId', () => {
    const acc = { accessoryId: 'acc-1', massKg: 5, mountingLocation: 'roof' };
    const withOne = calculatorReducer(INITIAL_STATE, {
      type: 'ADD_ACCESSORY',
      accessory: acc,
    });
    const withDup = calculatorReducer(withOne, {
      type: 'ADD_ACCESSORY',
      accessory: acc,
    });
  ...
  it('REMOVE_ACCESSORY removes by accessoryId', () => {
    ...
    let state = calculatorReducer(INITIAL_STATE, {
      type: 'ADD_ACCESSORY',
      accessory: acc1,
    });
  ...
  it('SET_VEHICLE_VARIANT with null clears accessories', () => {
    const acc = { accessoryId: 'acc-1', massKg: 5, mountingLocation: 'roof' };
    const withAcc = calculatorReducer(INITIAL_STATE, {
      type: 'ADD_ACCESSORY',
      accessory: acc,
    });
  ...
  it('ADD_CARAVAN_ACCESSORY adds a caravan accessory', () => {
    const acc = {
      accessoryId: 'cv-acc-1',
      massKg: 3,
      mountingLocation: 'front',
    };
    const next = calculatorReducer(INITIAL_STATE, {
      type: 'ADD_CARAVAN_ACCESSORY',
      accessory: acc,
    });
  ...
  it('REMOVE_CARAVAN_ACCESSORY removes by accessoryId', () => {
    ...
    const acc2 = {
      accessoryId: 'cv-acc-2',
      massKg: 5,
      mountingLocation: 'rear',
    };
    let state = calculatorReducer(INITIAL_STATE, {
      type: 'ADD_CARAVAN_ACCESSORY',
      accessory: acc1,
