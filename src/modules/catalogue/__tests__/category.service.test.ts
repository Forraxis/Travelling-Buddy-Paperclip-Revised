...
describe("Category CRUD", () => {
  it("create calls prisma.accessoryCategory.create", async () => {
    ...
    const result = await service.create({ name: "Roof Racks", slug: "roof-racks" });
...
describe("listHierarchy", () => {
  ...
  it("returns multiple roots when no parentId set", async () => {
    const root2 = makeCategory({ id: "cat-3", name: "Bull Bars", slug: "bull-bars" });
  ...
  it("handles deeply nested categories", async () => {
    ...
    const mid = makeCategory({ id: "m", name: "Mid", slug: "mid", parentId: "r" });
    const leaf = makeCategory({ id: "l", name: "Leaf", slug: "leaf", parentId: "m" });
