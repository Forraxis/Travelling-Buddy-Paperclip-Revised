...
import { createFitmentService, VEHICLE_BASE_LOCATIONS } from "../services/fitment.service";
...
describe("getAvailableMountingLocations", () => {
  ...
  it("adds provided locations when a bullbar is installed", async () => {
    ...
    const result = await service.getAvailableMountingLocations("vv-1", ["fit-1"]);
  ...
  it("supports multiple fitments providing multiple locations", async () => {
    prisma.accessoryFitment.findMany.mockResolvedValue([
      { mountingLocation: "ROOF_RACK", providesMountingLocations: ["CANOPY_ROOF"] },
      { mountingLocation: "REAR_BAR", providesMountingLocations: ["TOW_HITCH"] },
    ]);
    const result = await service.getAvailableMountingLocations("vv-1", ["fit-a", "fit-b"]);
...
describe("validateMountingCompatibility", () => {
  it("returns compatible=true when parent provides required location", async () => {
    prisma.accessoryFitment.findUniqueOrThrow
      ...
      .mockResolvedValueOnce({ providesMountingLocations: ["BULL_BAR", "TRAY_FLOOR"] });
    const result = await service.validateMountingCompatibility("fit-winch", "fit-bullbar");
  ...
  it("returns compatible=false when parent does not provide required location", async () => {
    ...
    const result = await service.validateMountingCompatibility("fit-winch", "fit-tray");
  ...
  it("returns compatible=false when parent provides nothing", async () => {
    ...
    const result = await service.validateMountingCompatibility("fit-winch", "fit-chassis");
    expect(result.compatible).toBe(false);
  ...
  it("returns all parentProvides in result for caller inspection", async () => {
    ...
    const result = await service.validateMountingCompatibility("fit-winch", "fit-bullbar");
    expect(result.parentProvides).toEqual(["BULL_BAR", "SNORKEL", "CANOPY_EXTERIOR"]);
  });
});
