...
export function VariantCard({ variant, href }: Props) {
  const content = (
    <div className="border-tb-neutral-200 flex flex-col gap-2 rounded-xl border bg-white p-4 transition-shadow hover:shadow-md">
      <div className="flex items-start justify-between gap-2">
        <span className="text-tb-primary font-semibold">{variant.name}</span>
      ...
      {isVehicleVariant(variant) ? (
        <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs text-gray-600 sm:grid-cols-3">
          <div>
            ...
            <span className="font-medium">
              {variant.gvmKg?.toLocaleString() ?? '—'} kg
            </span>
          </div>
          <div>
            <span className="text-gray-400">GCM</span>{' '}
            <span className="font-medium">
              {variant.gcmKg?.toLocaleString() ?? '—'} kg
            </span>
          </div>
          <div>
            <span className="text-gray-400">Tow cap.</span>{' '}
            <span className="font-medium">
              {variant.maxTowingCapacityKg?.toLocaleString() ?? '—'} kg
            </span>
          </div>
          <div>
            <span className="text-gray-400">Kerb</span>{' '}
            <span className="font-medium">
              {variant.kerbWeightKg?.toLocaleString() ?? '—'} kg
            </span>
          </div>
          <div>
            <span className="text-gray-400">TBM</span>{' '}
            <span className="font-medium">
              {variant.maxTowBallDownloadKg?.toLocaleString() ?? '—'} kg
            </span>
          </div>
          <div>
        ...
        <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs text-gray-600 sm:grid-cols-3">
          <div>
            ...
            <span className="font-medium">
              {(variant as CaravanVariantDto).atmKg?.toLocaleString() ?? '—'} kg
            </span>
          </div>
          <div>
            <span className="text-gray-400">GTM</span>{' '}
            <span className="font-medium">
              {(variant as CaravanVariantDto).gtmKg?.toLocaleString() ?? '—'} kg
            </span>
          </div>
          <div>
            <span className="text-gray-400">Tare</span>{' '}
            <span className="font-medium">
              {(variant as CaravanVariantDto).tareKg?.toLocaleString() ?? '—'}{' '}
              kg
            </span>
          </div>
          <div>
            <span className="text-gray-400">TBM</span>{' '}
            <span className="font-medium">
              {(variant as CaravanVariantDto).tbmKg?.toLocaleString() ?? '—'} kg
            </span>
          </div>
          <div>
            <span className="text-gray-400">Body</span>{' '}
            <span className="font-medium">
              {(variant as CaravanVariantDto).bodyLengthMm != null
                ? Math.round(
                    ((variant as CaravanVariantDto).bodyLengthMm! / 1000) * 10,
                  ) /
                    10 +
                  ' m'
                : '—'}
          ...
          <div>
            ...
            <span className="font-medium capitalize">
              {(variant as CaravanVariantDto).axleConfiguration
                .replace(/_/g, ' ')
                .toLowerCase()}
            </span>
          </div>
