...
export function iconForMount(location: string, label?: string | null): IconId {
  ...
  if (l === 'BULL_BAR' || l === 'REAR_BAR' || l === 'CARAVAN_BUMPER_BAR') return 'bar';
  ...
  if (l.startsWith('TUB') || l.startsWith('TRAY') || l.startsWith('CANOPY')) return 'drawer';
  return 'box';
}
...
export function AccessoryGlyph({
  ...
  switch (icon) {
    case 'drawer':
      return (
        <g>
          ...
          <line x1={x} y1={y + (2 * h) / 3} x2={x + w} y2={y + (2 * h) / 3} {...s} />
        </g>
      );
    case 'fridge':
      return (
        <g>
          ...
          <line x1={x + w * 0.5} y1={y + h * 0.12} x2={x + w * 0.5} y2={y + h * 0.32} {...s} />
        </g>
      );
    case 'water':
      return (
        <g>
          <rect x={x} y={y} width={w} height={h} rx={Math.min(w, h) * 0.35} {...s} {...fillFaint} />
          <path d={`M${x + w * 0.2},${cy} q${w * 0.15},-6 ${w * 0.3},0 t${w * 0.3},0`} {...s} />
        </g>
      );
    case 'jerry':
      return (
        <g>
          <rect x={x} y={y} width={w} height={h} rx={2} {...s} {...fillFaint} />
          <circle cx={x + w * 0.5} cy={y + h * 0.22} r={Math.min(w, h) * 0.1} {...s} />
        </g>
      );
    case 'spare':
      return (
        <g>
          <circle cx={cx} cy={cy} r={Math.min(w, h) / 2} {...s} {...fillFaint} />
    ...
    case 'bar':
      return <rect x={x} y={cy - h * 0.28} width={w} height={h * 0.56} rx={h * 0.28} {...s} {...fillFaint} />;
    case 'rack':
      return (
        <g>
          <rect x={x} y={y} width={w} height={h} rx={2} {...s} {...fillFaint} />
          <line x1={x + w / 3} y1={y} x2={x + w / 3} y2={y + h} {...s} />
          <line x1={x + (2 * w) / 3} y1={y} x2={x + (2 * w) / 3} y2={y + h} {...s} />
        </g>
      );
    case 'snorkel':
      return (
        <g>
          <circle cx={cx} cy={cy} r={Math.min(w, h) * 0.4} {...s} {...fillFaint} />
    ...
    case 'hitch':
      return (
        <g>
          <circle cx={cx} cy={cy} r={Math.min(w, h) * 0.32} {...s} {...fillFaint} />
    ...
    case 'toolbox':
      return (
        <g>
          <rect x={x} y={y + h * 0.2} width={w} height={h * 0.8} rx={2} {...s} {...fillFaint} />
          <path d={`M${x + w * 0.3},${y + h * 0.2} v${-h * 0.12} h${w * 0.4} v${h * 0.12}`} {...s} />
        </g>
      );
    case 'box':
    default:
      return (
        <g>
          <rect x={x} y={y} width={w} height={h} rx={2} {...s} {...fillFaint} />
          <line x1={x} y1={y} x2={x + w} y2={y + h} {...s} strokeOpacity={0.4} />
        </g>
      );
  }
}
