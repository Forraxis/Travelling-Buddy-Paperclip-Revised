  ...
  return (
    <div>
      <div className="flex items-center justify-between text-xs">
        <span className="text-tb-primary font-semibold">{label}</span>
        <span
          className={`font-bold tabular-nums ${status === 'fail' ? 'text-tb-danger' : status === 'warn' ? 'text-tb-warning' : 'text-tb-success'}`}
        >
          {pct < 0.5
            ? 'Balanced'
            : `${Math.round(Math.abs(imb))} kg ${imb > 0 ? 'right' : 'left'} (${pct.toFixed(0)}%)`}
        </span>
      </div>
      <div className="bg-tb-neutral-200 mt-1 flex h-2 overflow-hidden rounded-full">
        <div
          className="bg-tb-primary-light/70 h-full"
          style={{ width: `${(leftKg / total) * 100}%` }}
        />
        <div
          className="bg-tb-primary/70 h-full"
          style={{ width: `${(rightKg / total) * 100}%` }}
        />
      </div>
    </div>
  );
...
function AxleRow({
  ...
  const st = (v: number): MetricStatus =>
    v > limit ? 'fail' : v > limit * 0.95 ? 'warn' : 'ok';
  ...
  return (
    <div className="grid grid-cols-[68px_1fr_auto] items-center gap-2 text-[11px]">
      ...
      <div className="space-y-0.5">
        ...
        <div className="flex h-2 overflow-hidden rounded bg-gray-100">
          <div
            ...
            style={{ width: w(towing), backgroundColor: hex(st(towing)) }}
            title="Towing"
          />
        </div>
      </div>
      <span className="text-gray-600 tabular-nums">
        ...
        {delta !== 0 && (
          <span className={delta > 0 ? 'text-tb-danger' : 'text-tb-success'}>
            {' '}
            ({delta > 0 ? '+' : ''}
            {delta})
          </span>
...
export default function CoupledRigCanvas({
  ...
  const [selected, setSelected] = useState<{ id: string; side: Side } | null>(
    null,
  );
  ...
  const maxHalfW =
    Math.max(v.widthMm, c?.widthMm ?? 0, v.trackWidthMm) / 2 + 220;
  ...
  return (
    <div className="border-tb-neutral-200 to-tb-neutral-50 rounded-2xl border bg-gradient-to-b from-white p-3 shadow-sm">
      <svg
        ref={svgRef}
      ...
      {selected &&
        (() => {
          ...
          if (!dot) return null;
          return (
            <div className="border-tb-neutral-200 mt-2 flex flex-wrap items-center gap-2 rounded-lg border bg-white px-3 py-2 text-xs">
              <span className="text-tb-ink font-semibold">{dot.label}</span>
              ...
              {onRemove && (
                <button
                  type="button"
                  onClick={() => {
                    onRemove(selected.side, selected.id);
                    setSelected(null);
                  }}
                  className="border-tb-danger/30 text-tb-danger hover:bg-tb-danger/5 ml-auto rounded border px-2 py-0.5"
                >
                  Remove
                </button>
              )}
      ...
      {/* Balances */}
      <div className="border-tb-neutral-200 mt-3 grid gap-3 border-t pt-3 sm:grid-cols-2">
        {lat && (
          <Bar
            leftKg={lat.leftKg}
            rightKg={lat.rightKg}
      ...
      {c && (
        <div className="border-tb-neutral-200 mt-3 rounded-lg border bg-white p-3">
          <p className="text-tb-ink mb-2 text-xs font-semibold">
          ...
          <div className="space-y-1.5">
            <AxleRow
              label="Front axle"
              solo={soloFront}
              towing={result.vehicle.frontAxleKg}
              limit={result.vehicle.frontAxleLimitKg}
            />
            <AxleRow
              label="Rear axle"
              solo={soloRear}
              towing={result.vehicle.rearAxleKg}
              limit={result.vehicle.rearAxleLimitKg}
            />
          </div>
