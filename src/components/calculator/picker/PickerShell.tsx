      ...
      >
        ...
        <div className="flex flex-none items-center justify-between border-t border-tb-neutral-200 bg-white px-4 py-3 pb-[max(0.75rem,env(safe-area-inset-bottom))]">
          <button
            ...
            className="text-sm text-tb-primary-light underline-offset-2 hover:underline"
          >
            {config.label.charAt(0).toUpperCase() + config.label.slice(1)} not listed?{' '}
            <span className="font-medium">{config.submitLabel}</span>
          </button>
        </div>
      </div>
    </>
