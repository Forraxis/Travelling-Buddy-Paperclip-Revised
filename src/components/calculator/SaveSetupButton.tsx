...
export function SaveSetupButton({
  ...
  const { save, saving, canSave } = useSetupSave(setupId ?? null, {
    vehicleName,
    caravanName,
  });
  ...
  const buttonLabel =
    label ?? (saving ? 'Saving…' : isUpdate ? 'Update Setup' : 'Save Setup');
  ...
  return (
    <button
      ...
      className={
        className ??
        'bg-tb-primary hover:bg-tb-primary/90 rounded-lg px-4 py-2 text-sm font-medium text-white shadow-sm transition-colors disabled:cursor-not-allowed disabled:opacity-50'
      }
    >
      {buttonLabel}
    </button>
