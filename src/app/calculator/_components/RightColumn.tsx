...
function SkeletonBar({ widthClass = 'w-1/3' }: { widthClass?: string }) {
  return <div className={`h-2 rounded-full bg-tb-neutral-200 ${widthClass}`} />;
}

function LoadingState() {
  return (
    <div className="space-y-4">
      <div className="rounded-md bg-tb-neutral-200 px-4 py-3">
      ...
      <div className="rounded-lg border border-tb-neutral-200 bg-white p-4 space-y-3">
        {[1, 2, 3].map((i) => (
          <div key={i} className="flex items-center gap-3 py-2">
            ...
            <div className="flex-1 h-3 rounded-full bg-tb-neutral-200" />
  ...
  return (
    <div className="mb-4 rounded-lg border border-tb-neutral-200 bg-white p-4">
      <div className="mb-2 flex items-center justify-between">
        ...
        <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${statusBadgeColor(gvmStatus)}`}>
          {Math.round(pct)}%
        </span>
      </div>
      <div className="relative h-3 rounded-full bg-tb-neutral-200 overflow-hidden">
        <div
    ...
    <div className="mb-4 rounded-lg border border-tb-neutral-200 bg-white p-4">
      ...
      <div className="space-y-3">
        {result.recommendations.map((rec) => {
          const dotColor = rec.severity === 'critical' ? 'bg-red-500' : rec.severity === 'warn' ? 'bg-amber-400' : 'bg-blue-400';
          return (
...
function ResultsView({ result }: { result: PhysicsResult }) {
  return (
    <>
...
export default function RightColumn({ vehicleSelected = false, result }: RightColumnProps) {
  return (
