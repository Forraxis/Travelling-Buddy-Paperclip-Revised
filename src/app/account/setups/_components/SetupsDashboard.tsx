  ...
  if (setups.length === 0 && !search && selectedTags.size === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <Link
          href="/calculator"
          className="rounded-md bg-tb-primary px-5 py-2.5 text-sm font-medium text-white hover:bg-tb-primary-light"
        >
          Go to Calculator
        </Link>
      </div>
    ...
    <div>
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <input
          ...
          className="w-full rounded-md border border-tb-neutral-200 px-3 py-2 text-sm placeholder:text-gray-400 focus:border-tb-primary focus:outline-none focus:ring-1 focus:ring-tb-primary sm:w-64"
        />
      </div>
            ...
            >
              ...
              <div className="mb-3 flex flex-wrap gap-1">
                {setup.tags.map((tag) => (
                  <span
                    key={tag}
                    className="group inline-flex items-center gap-1 rounded-full bg-tb-primary-lighter px-2 py-0.5 text-xs font-medium text-tb-primary"
                  >
                    {tag}
                    <button
                      onClick={() => handleRemoveTag(setup.id, tag)}
                      className="hidden text-tb-primary/60 hover:text-tb-danger group-hover:inline"
                ...
                {tagInput === setup.id ? (
                  ...
                  >
                    <input
                      ...
                      placeholder="tag"
                      className="w-16 rounded-full border border-tb-neutral-200 px-2 py-0.5 text-xs focus:border-tb-primary focus:outline-none"
                    />
                  </form>
                ) : (
                  <button
                    onClick={() => setTagInput(setup.id)}
                    className="rounded-full border border-dashed border-gray-300 px-2 py-0.5 text-xs text-gray-400 hover:border-tb-primary hover:text-tb-primary"
                  >
                    +
                  </button>
                )}
              ...
              <div className="flex flex-wrap gap-2 border-t border-tb-neutral-200 pt-3">
                <Link
                  href={`/calculator?setupId=${setup.id}`}
                  className="rounded-md bg-tb-primary px-3 py-1.5 text-xs font-medium text-white hover:bg-tb-primary-light"
                >
                  Open
                </Link>
                <button
                  onClick={() => handleDuplicate(setup)}
                  className="rounded-md border border-tb-neutral-200 px-3 py-1.5 text-xs font-medium text-gray-700 hover:bg-gray-50"
                >
                  Duplicate
                </button>
                <button
          ...
          <div className="w-full max-w-sm rounded-lg bg-white p-6 shadow-xl">
            ...
            <div className="flex justify-end gap-3">
              <button
                ...
                className="rounded-md border border-tb-neutral-200 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50"
              >
                Cancel
              </button>
              <button
