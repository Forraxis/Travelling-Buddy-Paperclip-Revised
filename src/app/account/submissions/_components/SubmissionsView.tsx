...
export function SubmissionsView({ submissions, trustTier, untilTrusted, showSuccessBanner }: Props) {
    ...
    <div className="space-y-6">
      ...
      {bannerVisible && (
        <div className="flex items-start gap-3 rounded-xl border border-green-200 bg-green-50 px-4 py-3">
          <svg className="mt-0.5 h-5 w-5 shrink-0 text-green-600" viewBox="0 0 20 20" fill="currentColor">
            <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
          </svg>
          <div className="flex-1">
            <p className="text-sm font-medium text-green-800">Submission received</p>
            <p className="mt-0.5 text-xs text-green-700">
              Your submission is now in the review queue. You can track its status below.
            </p>
          </div>
          <button
            type="button"
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          {s.status === "APPROVED" && s.catalogueUrl && (
            <Link
              href={s.catalogueUrl}
              className="mt-1 inline-flex items-center gap-1 text-xs font-medium text-tb-primary hover:underline"
            >
              View in catalogue
          ...
          {s.status === "REJECTED" && (
            <div className="mt-1 space-y-1">
              ...
              {url && (
                <Link
                  href={url}
                  className="inline-flex items-center gap-1 text-xs font-medium text-tb-primary hover:underline"
                >
                  Edit and resubmit
                  <svg className="h-3 w-3" viewBox="0 0 20 20" fill="currentColor">
          ...
          {s.status === "DRAFT" && s.draftExpiresAt && (
            <p className="mt-1 text-xs text-gray-400">
              Auto-saved draft · expires {new Date(s.draftExpiresAt).toLocaleDateString("en-AU")}
            </p>
          )}
        </div>
