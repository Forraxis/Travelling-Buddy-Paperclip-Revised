...
function bodyTypeLabel(bodyType: CaravanBodyType): string {
  switch (bodyType) {
    case "CARAVAN_POP_TOP": return "pop-top caravan";
    case "CARAVAN_FULL_HEIGHT": return "full-height caravan";
    case "OFF_ROAD_CARAVAN": return "off-road caravan";
    case "CAMPER_TRAILER": return "camper trailer";
    case "HYBRID": return "hybrid camper";
    case "FIFTH_WHEELER": return "fifth-wheeler";
    default: return "caravan";
  }
}
    ...
    itemListElement: variants.map((v, i) => {
      ...
      return {
        ...
        item: {
          ...
          ...(v.atmKg != null
            ? { weightTotal: { "@type": "QuantitativeValue", value: v.atmKg, unitCode: "KGM" } }
            : {}),
      ...
      <table className="w-full min-w-[640px] text-sm">
        <thead>
          <tr className="border-b border-gray-200 bg-gray-50">
            ...
            <th className="px-4 py-2.5 text-right font-semibold text-gray-700">ATM (kg)</th>
            <th className="px-4 py-2.5 text-right font-semibold text-gray-700">GTM (kg)</th>
            <th className="px-4 py-2.5 text-right font-semibold text-gray-700">TBM (kg)</th>
          ...
          {groups.map((group) =>
            group.rows.map((row, rowIdx) => (
              ...
              >
                ...
                <td className="px-4 py-2.5 text-right tabular-nums text-gray-900">{formatKg(row.atmKg)}</td>
                <td className="px-4 py-2.5 text-right tabular-nums text-gray-900">{formatKg(row.gtmKg)}</td>
                <td className="px-4 py-2.5 text-right tabular-nums text-gray-900">{formatKg(row.tbmKg)}</td>
      ...
      <div className="mx-auto max-w-5xl px-4 py-10 sm:px-6">
        ...
        <p className="mt-4 text-base leading-relaxed text-gray-600">
          ...
          {distinctAxles.length > 0 && (
            <>, available in {listProse(distinctAxles)} configuration{distinctAxles.length !== 1 ? "s" : ""}</>
          )}
          {variantCount > 0 && (
            <>
              {". "}
              ...
              , each with ATM, GTM, and tow ball mass specifications for Australian-market caravans.
            </>
          )}
        </p>
        ...
        <div className="mt-10 rounded-xl border border-blue-100 bg-blue-50 p-6">
          ...
          <p className="mt-1 text-xs text-blue-700">
            Select your exact variant above to view its full ATM, GTM, and tow ball mass
            specifications, then use the TravellingBuddy calculator to check your towing compliance.
          </p>
          <Link
            href="/calculator/"
