...
interface Props {
  ...
  searchParams: Promise<{ axleConfiguration?: string; year?: string; cursor?: string }>;
}
...
export default async function CaravanVariantsPage({ params, searchParams }: Props) {
  const { makeSlug, modelSlug } = await params;
  ...
  if (axleConfiguration || year !== undefined) {
    const result = await service.listVariantsFiltered(
      {
        ...(axleConfiguration
          ? { axleConfiguration: axleConfiguration as Parameters<typeof service.listVariantsFiltered>[0]["axleConfiguration"] }
          : {}),
        ...(year !== undefined ? { year } : {}),
      },
  ...
  return (
    <div className="space-y-6">
      ...
      <Suspense>
        <FilterChips paramName="axleConfiguration" options={AXLE_CONFIGS} label="Axle" />
      </Suspense>

      {variants.length === 0 ? (
        <div className="rounded-xl border border-tb-neutral-200 py-16 text-center text-gray-400">
          {axleConfiguration || year ? "No variants match the selected filters." : "No variants available."}
        </div>
      ) : (
