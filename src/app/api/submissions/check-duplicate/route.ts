...
export async function GET(request: Request) {
  ...
  if (type === "vehicle") {
    ...
    const result = await checkVehicleDuplicateByText({ makeName, modelName, year });
  ...
  if (type === "caravan") {
    ...
    const result = await checkCaravanDuplicateByText({ makeName, modelName, year });
    return NextResponse.json(result);
  }
  ...
  if (type === "accessory") {
    ...
    const result = await checkAccessoryDuplicateByText({ brandName, modelName });
    return NextResponse.json(result);
  }
