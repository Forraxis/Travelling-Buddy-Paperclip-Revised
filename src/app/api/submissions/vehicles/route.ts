...
function fuelTypeEnum(fuelType: string): "PETROL" | "DIESEL" | "HYBRID" | "ELECTRIC" {
...
function bodyTypeEnum(
  bodyType: string
): "DUAL_CAB_UTE" | "SINGLE_CAB_UTE" | "EXTRA_CAB_UTE" | "WAGON" | "SUV" | "VAN" | "TROOPCARRIER" | "OTHER" {
  const map: Record<string, "DUAL_CAB_UTE" | "SINGLE_CAB_UTE" | "EXTRA_CAB_UTE" | "WAGON" | "SUV" | "VAN" | "TROOPCARRIER" | "OTHER"> = {
...
export async function POST(request: Request) {
  ...
  const [submission, communityVariant] = await prisma.$transaction(async (tx) => {
    ...
    if (!resolvedModelId && data.newModelName) {
      const modelSlug = toSlug(`${data.newModelName}-${Math.random().toString(36).slice(2, 6)}`);
    ...
    const newVariant = await tx.vehicleVariant.create({
      data: {
        ...
        frontOverhangMm: data.frontOverhangMm ? Math.round(data.frontOverhangMm) : null,
        rearOverhangMm: data.rearOverhangMm ? Math.round(data.rearOverhangMm) : null,
        totalLengthMm: data.totalLengthMm ? Math.round(data.totalLengthMm) : null,
        fuelTankCapacityL: data.fuelTankLitres ? Math.round(data.fuelTankLitres) : null,
