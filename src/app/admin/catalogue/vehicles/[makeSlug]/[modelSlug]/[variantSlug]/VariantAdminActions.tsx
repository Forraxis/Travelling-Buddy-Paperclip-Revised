...
import {
  splitVariantRangeAction,
  advanceYearToAction,
  closeCurrentProductionAction,
...
export function VariantAdminActions({ variant, makeSlug, modelSlug }: Props) {
  ...
  async function handleSplit() {
    ...
    setBusy(true);
    const result = await splitVariantRangeAction(variant.id, yf, yt);
    setBusy(false);
    if (result.success) {
      toast(`Split into ${result.data.created} variants`);
      setSplitOpen(false);
      router.push(`/admin/catalogue/vehicles/${makeSlug}/${modelSlug}`);
      router.refresh();
    } else {
  ...
  async function handleAdvance() {
    ...
    setBusy(true);
    const result = await advanceYearToAction(variant.id, yt);
    setBusy(false);
    if (result.success) {
  ...
  async function handleClose() {
    setBusy(true);
    const result = await closeCurrentProductionAction(variant.id);
    setBusy(false);
    if (result.success) {
  ...
  return (
    <div className="rounded-lg border border-amber-200 bg-amber-50 p-6">
      <h3 className="mb-1 text-sm font-semibold tracking-wide text-amber-700 uppercase">
        Admin Actions
      </h3>
      <p className="mb-4 text-xs text-amber-600">
        These actions modify year ranges, regenerate slugs, and create audit
        records.
      </p>
      ...
      >
        <p className="mt-2 text-sm text-gray-600">
          ...
          <strong>
            {variant.yearFrom}–
            {variant.isCurrentProduction ? 'current' : variant.yearTo}
          </strong>
        </p>
        <p className="mt-1 text-sm text-gray-500">
          Enter the anomaly year(s). New variants will be created for each
          segment and the source row will be deleted.
        </p>
        <div className="mt-4 grid grid-cols-2 gap-3">
          <div>
            ...
            <input
              ...
              className="border-tb-neutral-200 focus:ring-tb-primary w-full rounded-lg border px-3 py-2 text-sm focus:ring-2 focus:outline-none"
            />
          </div>
          <div>
            <label className="mb-1 block text-xs font-medium text-gray-700">
      ...
      >
        ...
        <p className="mt-1 text-sm text-gray-500">
          Sets a new year_to, marks the variant as no longer current production,
          regenerates the slug, and creates a 301 redirect.
        </p>
        <div className="mt-4">
          <label className="mb-1 block text-xs font-medium text-gray-700">
            New year_to
        ...
        {advanceYearTo && !isNaN(parseInt(advanceYearTo)) && (
          <p className="mt-2 text-xs text-gray-500">
            New range will be:{' '}
            <strong>
              {variant.yearFrom}–{advanceYearTo}
            </strong>
          </p>
        )}
        <div className="mt-6 flex justify-end gap-3">
      ...
      >
        ...
        <ul className="mt-1 list-disc space-y-1 pl-5 text-sm text-gray-600">
          <li>
            Set <code>isCurrentProduction = false</code>
          </li>
          <li>
            Lock <code>yearTo = {currentYear}</code>
          </li>
          <li>
            Regenerate slug to{' '}
            <code>
              …{variant.yearFrom}-{currentYear}
            </code>
          </li>
