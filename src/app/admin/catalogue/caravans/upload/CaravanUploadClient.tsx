...
export function CaravanUploadClient() {
  ...
  async function processFile(file: File) {
    ...
    const result = await previewCaravanUploadAction(text);
    setLoading(false);
  ...
  async function handleCommit() {
    ...
    setLoading(true);
    const result = await commitCaravanUploadAction(csvText);
    setLoading(false);
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-lg border border-tb-neutral-200 bg-tb-neutral-50 p-4 text-xs text-gray-500">
        ...
        <p>
          ...
          axle_configuration, atm_kg, gtm_kg, tare_kg, tbm_kg,
          coupling_to_axle_mm, body_length_mm, overall_length_mm,
          fresh_water_capacity_l, grey_water_capacity_l
        </p>
        <p className="mt-1 font-medium text-gray-700">Optional:</p>
        <p>
          make_country_of_origin, is_current_production (true/false), market
          (AU/NZ/US/EU/GB, default AU), axle_spacing_mm, gas_bottle_config
        </p>
        <p className="mt-1">
          body_type values: CARAVAN_POP_TOP, CARAVAN_FULL_HEIGHT,
          OFF_ROAD_CARAVAN, CAMPER_TRAILER, HYBRID, FIFTH_WHEELER, OTHER
        </p>
        <p>
          axle_configuration values: SINGLE_AXLE, DUAL_AXLE_CLOSE_COUPLED,
          DUAL_AXLE_SPREAD, TRIPLE_AXLE
        </p>
      </div>
    </div>
  );
...
function PreviewSummary({ preview }: { preview: CaravanCsvPreviewResult }) {
  return (
...
function PreviewRow({ row }: { row: CaravanCsvRowResult }) {
