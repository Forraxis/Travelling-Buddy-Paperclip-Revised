  ...
  return (
    ...
    >
      ...
      >
        ...
        <p className="mb-4 truncate text-sm text-gray-500">{entityName}</p>
...
export function ModerationQueueView({ submissions }: Props) {
  ...
  const [filterAge, setFilterAge] = useState<'all' | '1d' | '3d' | '7d+'>(
    'all',
  );
  const [search, setSearch] = useState('');
  const [rejectTarget, setRejectTarget] = useState<UnifiedSubmission | null>(
    null,
  );
  ...
  const reviewQueue = useMemo(
    () =>
      submissions
        ...
        .sort(
          (a, b) =>
            verdictPriority(a.vlmVerdict) - verdictPriority(b.vlmVerdict),
        ),
    [submissions],
  );
  ...
  const filtered = useMemo(() => {
    ...
    if (filterTier !== 'all')
      list = list.filter((s) => s.submitter.trustTier === filterTier);
    if (filterVerdict !== 'all')
      list = list.filter((s) => (s.vlmVerdict ?? 'none') === filterVerdict);
    if (filterAge === '1d') list = list.filter((s) => getAge(s.createdAt) <= 1);
    else if (filterAge === '3d')
      list = list.filter((s) => getAge(s.createdAt) <= 3);
    else if (filterAge === '7d+')
      list = list.filter((s) => getAge(s.createdAt) >= 7);
    if (search.trim()) {
      const q = search.toLowerCase();
      list = list.filter(
        (s) =>
          s.entityName.toLowerCase().includes(q) ||
          (s.submitter.name ?? '').toLowerCase().includes(q),
      );
    }
    return list;
  ...
  const chipInactive =
    'border-gray-300 bg-white text-gray-600 hover:border-tb-primary hover:text-tb-primary';
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">
            {reviewQueue.length} pending review · {spotcheckQueue.length}{' '}
            spot-check
          </p>
        </div>
      </div>
        ...
        {(['review', 'spotcheck'] as const).map((t) => (
          ...
          >
            {t === 'review'
              ...
              : `Spot-check (${spotcheckQueue.length})`}
          </button>
        ))}
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        <input
          ...
          className="focus:ring-tb-primary w-56 rounded-md border border-gray-300 px-3 py-1.5 text-sm focus:ring-2 focus:outline-none"
        />
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      ...
                      >
                        <div className="mb-0.5 flex items-center gap-2">
                          <span
                            className={`rounded px-1.5 py-0.5 text-xs font-medium ${TYPE_COLORS[sub.type]}`}
                          >
                            {sub.type}
                          </span>
                    ...
                    <td className="px-4 py-3">
                      <div className="text-sm text-gray-800">
                        {sub.submitter.name ?? 'Unknown'}
                      </div>
                      <span
                        className={`inline-block rounded px-1.5 py-0.5 text-xs font-medium ${TRUST_COLORS[sub.submitter.trustTier] ?? 'bg-gray-100 text-gray-500'}`}
                      >
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <Link
                          href={`/admin/moderation/${sub.id}?type=${sub.type}`}
                          className="rounded-md border border-gray-300 px-3 py-1.5 text-xs font-medium whitespace-nowrap text-gray-700 hover:bg-gray-50"
                        >
                          Review
                        </Link>
                        <button
                          ...
                          className="rounded-md bg-green-600 px-3 py-1.5 text-xs font-medium whitespace-nowrap text-white hover:bg-green-700 disabled:opacity-60"
                        >
                          {busyApprove === sub.id ? '…' : 'Approve'}
                        </button>
                        <button
                          onClick={() => setRejectTarget(sub)}
                          className="rounded-md bg-red-600 px-3 py-1.5 text-xs font-medium whitespace-nowrap text-white hover:bg-red-700"
                        >
                          Reject
                        </button>
                      </div>
