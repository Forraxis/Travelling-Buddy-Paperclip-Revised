...
const STATE_GUIDANCE_DIR = path.join(
  process.cwd(),
  'src/content/state-guidance',
);
...
export const STATE_AUTHORITY: Record<
  AUStateCode,
...
> = {
  ...
  qld: {
    name: 'Transport and Main Roads QLD',
  ...
  sa: {
    ...
    url: 'https://dit.sa.gov.au/',
  },
  tas: {
    name: 'Department of State Growth TAS',
    url: 'https://www.transport.tas.gov.au/',
  },
  nt: {
    name: 'Department of Infrastructure, Planning and Logistics NT',
    url: 'https://nt.gov.au/driving',
  },
  act: {
...
export function getAllStateGuidanceParams(): {
  stateCode: string;
  topic: string;
}[] {
