...
import { resolveVehiclePositionMm, resolveCaravanPositionMm } from "./position-map";
...
export function generateRecommendations(
  ...
  if (vr.gvmStatus !== "ok") {
    ...
    recs.push({
      ...
      actions: vr.gvmStatus === "fail"
        ? [{ label: "GVM upgrade kits for this vehicle", type: "affiliate_search" }]
        : undefined,
    });
  }
  ...
  if (vr.rearAxleStatus !== "ok") {
    ...
    const contributorItems = vehicleAccessories.map((a) => {
      const posX = a.cogXMm != null
        ? a.cogXMm
        : resolveVehiclePositionMm(a.mountingLocation, vehicle);
      // Items at or behind rear axle (posX <= 0) apply rearward lever effect
      const momentContrib = resolvedWeight(a) * (vehicle.wheelbaseMm - posX) / vehicle.wheelbaseMm;
      return { location: a.mountingLocation, weight: resolvedWeight(a), contrib: momentContrib };
    });
    contributorItems.sort((a, b) => b.contrib - a.contrib);
    const topRear = contributorItems.slice(0, 2).map(
      (c) => `${c.location} (${Math.round(c.weight)} kg)`
    );

    const hasTowBall = !!caravan && vr.towBallDownloadKg != null && vr.towBallDownloadKg > 200;
    const bodyLines = [
    ...
    if (topRear.length) bodyLines.push(`Primary contributors: ${topRear.join(", ")}.`);
    if (hasTowBall)
      bodyLines.push(
    ...
    recs.push({
      ...
      metric: "rearAxle",
      title: vr.rearAxleStatus === "fail" ? "Rear axle overloaded" : "Rear axle approaching limit",
      body: bodyLines.join(" "),
      actions: hasTowBall
  ...
  if (
    ...
    recs.push({
      ...
      actions: [{ label: "Weight distribution hitches", type: "affiliate_search" }],
    });
  }
  ...
  if (vr.towBallPctStatus && vr.towBallPctStatus !== "ok" && vr.towBallPctOfAtm! < 9) {
  ...
  if (vr.towBallPctStatus && vr.towBallPctStatus !== "ok" && vr.towBallPctOfAtm! > 11) {
    const caravanAccessories = input.caravanAccessories ?? [];
  ...
  if (cr.atmStatus !== "ok") {
    ...
    recs.push({
      ...
      metric: "atm",
      title: cr.atmStatus === "fail" ? "Caravan ATM exceeded" : "Caravan ATM approaching limit",
      body: cr.atmStatus === "fail"
  ...
  if (cr.gtmStatus !== "ok") {
    ...
    recs.push({
      ...
      metric: "gtm",
      title: cr.gtmStatus === "fail" ? "Caravan axle(s) overloaded" : "Caravan axle(s) approaching limit",
      body: cr.gtmStatus === "fail"
